<template>
  ...
  >
        ...
        <div class="col-sm-12">
          <div class="form-check form-check-inline">
            ...
            <label
              ...
              :label="`${tagid}lktype-ch1_label`"
              :for="`${tagid}lktype-ch1`"
              >Private Cash</label
            >
          </div>
          <div class="form-check form-check-inline">
            ...
            <label
              class="form-check-label"
              :label="`${tagid}lktype-ch2_label`"
              :for="`${tagid}lktype-ch2`"
              >Wallet</label
            >
          </div>
        </div>
    ...
    <Dialog
      v-if="visibleAccDialog"
      :title-render="customTitleBar"
      :width="800"
      @close="toggleAccDialog"
    >
      <template v-slot:accModalTitleBar="">
        <h5
          v-if="accLookuptype === 'P'"
          :id="`${tagid}lk_modal_title_P`"
          class="modal-title"
        >
          Account Lookup
        </h5>
        <h5
          v-else-if="accLookuptype === 'W'"
          :id="`${tagid}lk_modal_title_W`"
          class="modal-title collapse"
        >
          Wallet Lookup
        </h5>
      </template>
...
export default {
  ...
  methods: {
    async queryLookupAccount() {
      await axios
        .post(
          `${this.baseURLUtil}/lookupaccount/collect`,
          this.data,
          this.headers
        )
        .then((result) => {
